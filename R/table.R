...
## Define class aafTable
    ...
    setClass("aafTable", representation(probeids = "character",
             ...
             prototype = list(probeids = character(0),
                              table = list()), where = where)
    
    ## Constructors for aafTable
                              
    assign("aafTable", function(..., items = list(...), colnames = names(items), 
    ...
    assign("aafTableAnn", function(probeids, chip, colnames = aaf.handler(),
                                   widget = FALSE) {
    ...
    assign("dim.aafTable", function(x) {
    
        return(c(length(x@table[[1]]), length(x@table)))
    
    }, envir = where)
    
    assign("merge.aafTable", function(x, y, suffixes = c(".x",".y"), ...) {
    ...
    assign("rbind.aafTable", function(..., deparse.level = 1)  {
    ...
    assign("$.aafTable", function(x, val) {

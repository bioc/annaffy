...
%\VignetteDepends{Biobase, GO, KEGG, multtest, hgu95av2}
...
contained in Bioconductor data packages. The data packages are
created by another package called \verb+AnnBuilder+. It gathers
annotation data from many diverse sources and makes the information
easily processed by R. Preconstructed packages for most Affymetrix
chips are available on the Bioconductor web site.
...
(For the purpose of demonstration, \verb+annaffy+ can act as its
own data package. It contains a subset of 250 random genes selected
from the \verb+hgu95av2+ data package.)

<<>>=
probeids <- ls(annaffySYMBOL)
symbols <- aafSymbol(probeids, "annaffy")
symbols[1:3]
symbols[[4]]
@
...
<<>>=
gbs <- aafGenBank(probeids, "annaffy")
getURL(gbs[[1]])
@
...
possible. LocusLink is a very useful online database that links to
many other data sources not referenced by Bioconductor. One worthy
of note is OMIM, which provides relatively concise gene function
and mutant phenotype information. (See this URL
\href{http://www.ncbi.nlm.nih.gov/LocusLink/LocRpt.cgi?l=2322}{here}.)

<<>>=
lls <- aafLocusLink(probeids, "annaffy") 
getURL(lls[[2]])
@
...
<<>>=
bands <- aafCytoband(probeids, "annaffy") 
getURL(bands[[2]])
@
...
\href{http://www.ncbi.nih.gov/entrez/query.fcgi?tool=bioconductor&cmd=Retrieve&db=PubMed&list_uids=12393674%2c12239147%2c12239146%2c12070009%2c12060771%2c12036858%2c11971190%2c8394751%2c7507245}{here}.)

<<>>=
pmids <- aafPubMed(probeids, "annaffy")
getURL(pmids[[2]])
@
...
entire at once, including multiple GO ids. The usefulness of such
a query may be dubious, but it is possible. See this URL
\href{http://godatabase.org/cgi-bin/go.cgi?open_0=GO:0005021&open_0=GO:0005524&open_0=GO:0004872&open_0=GO:0007169&open_0=GO:0008284&open_0=GO:0006468&open_0=GO:0005887&open_0=GO:0016740}{here}.
...
this URL
\href{http://godatabase.org/cgi-bin/go.cgi?open_0=GO:0007169}{here}.)
...
<<>>=
paths <- aafPathway(probeids, "annaffy")
getURL(paths[[5]])
@
...
The rest of this tutorial will make use of an \verb+exprSet+
...
<<>>=
anntable <- aafTableAnn(probeids[1:50], "annaffy", anncols)
@
...
For the purposes of demonstration, we will use the hgu95av2 annotation
data package available through Bioconductor.

<<>>=
library(hgu95av2)
probeids <- ls(hgu95av2SYMBOL)
gos <- aafGO(probeids[1:2], "hgu95av2")
getText(gos)
@
...
documentation\footnote{\url{http://www.perldoc.com/perl5.8.0/pod/perlre.html}}
...
<<>>=
aafSearchGO("hgu95av2", c("GO:0000002", "GO:0000008"))
aafSearchGO("hgu95av2", c("2", "8"))
aafSearchGO("hgu95av2", c(2, 8))
@

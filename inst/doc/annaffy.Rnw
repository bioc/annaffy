...
%\VignetteDepends{Biobase, GO, KEGG, multtest, hgu95av2db}
...
(For the purpose of demonstration, we will use the \verb+hgu95av2db+
metadata package and probe ids from the \verb+aafExpr+ dataset.)

<<>>=
data(aafExpr)
probeids <- featureNames(aafExpr)
symbols <- aafSymbol(probeids, "hgu95av2db")
symbols[[54]]
symbols[55:57]
@
...
<<>>=
gbs <- aafGenBank(probeids, "hgu95av2db")
getURL(gbs[[1]])
@
...
<<>>=
lls <- aafLocusLink(probeids, "hgu95av2db") 
getURL(lls[[2]])
@
...
<<>>=
bands <- aafCytoband(probeids, "hgu95av2db") 
getURL(bands[[2]])
@
...
<<>>=
pmids <- aafPubMed(probeids, "hgu95av2db")
getURL(pmids[[2]])
@
...
<<>>=
paths <- aafPathway(probeids, "hgu95av2db")
getURL(paths[[5]])
@
...
<<>>=
anntable <- aafTableAnn(probeids[1:50], "hgu95av2db", anncols)
@
...
For the purposes of demonstration, we will use the hgu95av2db annotation
data package available through Bioconductor.

<<>>=
library(hgu95av2db)
probeids <- ls(hgu95av2SYMBOL)
gos <- aafGO(probeids[1:2], "hgu95av2db")
getText(gos)
@
...
<<>>=
kinases <- aafSearchText("hgu95av2db", "Description", "kinase")
kinases[1:5]
...
<<>>=
probes <- aafSearchText("hgu95av2db", c("Description", "Pathway"),
...
<<>>=
probes <- aafSearchText("hgu95av2db", "Description",
...
aafSearchText("hgu95av2db", "GenBank", gbs)
@
...
<<>>=
aafSearchGO("hgu95av2db", c("GO:0000002", "GO:0000008"))
aafSearchGO("hgu95av2db", c("2", "8"))
aafSearchGO("hgu95av2db", c(2, 8))
@
